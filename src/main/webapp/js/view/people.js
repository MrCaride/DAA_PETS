var PeopleView = (function() {
	...
	var createPersonRow = function(person) {
		return '<tr id="person-'+ person.id +'" class="row">\
			...
			<td class="col-sm-3">\
				<a class="edit btn btn-primary me-1" href="#">Editar</a>\
				<a class="delete btn btn-warning me-1" href="#">Eliminar</a>\
				<a class="manage-pets btn btn-info" href="#">Gestionar mascotas</a>\
			</td>\
		</tr>';
	};
	...
	var addRowListeners = function(person) {
		...
		$('#person-' + person.id + ' a.manage-pets').click(function(event) {
			event.preventDefault();
			$('#people-container').hide();
			$('#pets-container').show();
			new PetView(new PetDAO(), person.id, 'pets-container', 'pets-container').init();
		});
	};

    var appendToTable = function(person) {
        $(listQuery + ' > tbody:last').append(createPersonRow(person));
